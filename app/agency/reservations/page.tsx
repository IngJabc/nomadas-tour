...
export default function AgencyReservationsPage() {
  ...
  const getPassengersText = (r: Reservation) => {
    ...
    const codes = r.reservation_passengers?.map(p => p.seats?.seat_code ?? '').filter(Boolean).join(', ') ?? '';
    return { count, codes };
  };
  ...
  const filtered = reservations.filter((r) => {
    ...
    if (search) {
      ...
      const matchPassenger = passengers.some(p => p.name.toLowerCase().includes(q) || p.document.toLowerCase().includes(q));
      return (
  ...
  const boardedCount = reservations.filter((r) => r.status === 'boarded').length;
                    ...
                    return (
                      <tr key={r.id} className="hover:bg-slate-50 transition-colors border-b border-slate-100">
                        ...
                        <td className="px-5 py-4 font-[family-name:var(--font-body)] font-normal text-[13px] text-[var(--color-brand-muted)] whitespace-nowrap">
                          ...
                          <br />
                          <span className="text-[11px]">{r.trips?.departure_time ? format(new Date(r.trips.departure_time), 'dd/MM HH:mm') : ''}</span>
                        </td>
                        <td className="px-5 py-4 whitespace-nowrap">
                ...
                <Link key={r.id} href={`/agency/reservations/${r.id}`} className="block no-underline">
                  <div className="bg-[var(--color-brand-surface)] rounded-2xl p-4 border border-[rgba(0,0,0,0.06)] shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
                    ...
                    <div className="flex items-center gap-3 text-xs text-[var(--color-brand-muted)] mb-3">
                      <span className="font-[family-name:var(--font-body)] text-[13px]">{count} {count === 1 ? 'pasajero' : 'pasajeros'}</span>
