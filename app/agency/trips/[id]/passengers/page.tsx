...
import { ArrowLeft, Bus, Calendar, Clock, Users, AlertTriangle, Search, UserCheck } from 'lucide-react';
import { agencyApi } from '@/lib/api';
import type { AgencyTripPassengersResponse, AgencyTripPassenger } from '@/types';
...
const VEHICLE_LABELS: Record<string, string> = { bus: 'Autobús', kia: 'KIA', van: 'Van', microbús: 'Microbús' };
...
export default function TripPassengersPage() {
    ...
    return (
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div key={i} className="flex items-center gap-3 p-3 bg-white rounded-xl border border-[rgba(0,0,0,0.06)] animate-pulse">
  ...
  if (fetchError) {
    return (
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        <PageHeader title="Pasajeros del viaje" breadcrumbs={[{ label: 'Viajes', href: '/agency/trips' }]} />
        <motion.div
      ...
      <motion.div variants={pageFade} initial="hidden" animate="visible" transition={{ duration: 0.25 }}>
        <PageHeader
          ...
          breadcrumbs={[
            ...
            { label: trip?.route?.destination ?? 'Viaje', href: `/agency/trips/${tripId}` },
          ]}
          action={
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-[13px] text-[var(--color-brand-muted)]">
                  ...
                  <span className="inline-flex items-center gap-1.5">
                    ...
                    {VEHICLE_LABELS[trip.vehicle_type] ?? trip.vehicle_type} · {trip.total_seats} puestos
                  </span>
                </div>
              </div>
      ...
      {filtered.length === 0 ? (
        ...
        >
          <EmptyState
            ...
            message={search ? 'No se encontraron pasajeros con ese criterio' : 'Este viaje aún no tiene pasajeros'}
            action={
              !search
                ? { label: 'Crear reserva', href: `/agency/reservations/new?trip=${tripId}&source=trips` }
                : undefined
            }
          />
        </motion.div>
