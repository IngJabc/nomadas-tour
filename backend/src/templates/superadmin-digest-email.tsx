...
export interface SuperadminDigestTripRow {
  route_label: string;
...
interface SuperadminDigestEmailProps {
  digestDate: string;
  ...
  upcomingTrips: SuperadminDigestTripRow[];
  occupancyByTrip: SuperadminDigestOccupancyRow[];
  dashboardUrl: string;
}

export function SuperadminDigestEmail({
  digestDate,
  totalAgencies,
  activeAgencies,
  activeTrips,
      ...
      <body style={bodyStyle}>
        <div style={containerStyle}>
          <div style={cardStyle}>
            ...
            <h1 style={titleStyle}>Resumen operativo diario — plataforma</h1>

            <p style={textStyle}>
              Este es el resumen global de la operación para el día{' '}
              <strong>{digestDate}</strong>.
            </p>
            ...
            <div style={infoBoxStyle}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <tbody>
                  <tr>
                    <td style={infoLabelStyle}>Agencias</td>
                    <td style={infoValueStyle}>
                      {activeAgencies}/{totalAgencies} activas
                    </td>
                  </tr>
                  <tr>
            ...
            <h2 style={sectionTitleStyle}>Ocupación reciente</h2>

            {occupancyByTrip.length === 0 ? (
              <p style={textStyle}>No hay viajes recientes para el snapshot de ocupación.</p>
            ) : (
              <div style={infoBoxStyle}>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr>
                      <th style={thStyle}>Ruta</th>
                      <th style={thStyle}>Reservados</th>
                      <th style={thStyle}>Ocup.</th>
                    </tr>
                  </thead>
                  <tbody>
                    {occupancyByTrip.map((row) => (
                      <tr key={`${row.label}-${row.departure}`}>
                        <td style={tdStyle}>{row.label}</td>
                        <td style={tdStyle}>
                          {row.reserved}/{row.total}
                        </td>
                        <td style={tdStyle}>{row.occupancy_pct}%</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
