-- ============================================================
-- 066_create_agency_reservation_departed.sql
-- Reject new reservations when trips.departure_time <= NOW()
-- even if status = 'active'.
--
-- CREATE OR REPLACE only; same signature as 014/047.
-- No new tables, columns, indexes, triggers, policies, or RPCs.
-- Reaffirm EXECUTE grants (service_role only, per 037/047).
-- ============================================================
...
BEGIN
  -- 1. Validar que el viaje existe, está active y aún no ha salido.
  --    Una sola lectura: conserva ERR_TRIP_NOT_FOUND vs ERR_TRIP_DEPARTED.
  ...
  -- 9. Retornar resultado (ticket_code additive; existing keys unchanged)
...
COMMENT ON FUNCTION public.create_agency_reservation(
  UUID, UUID, UUID, TEXT, TEXT, TEXT, UUID[], TEXT[], TEXT[], TEXT[]
) IS
  'Atomic agency reservation create. Rejects departure_time <= NOW() with ERR_TRIP_DEPARTED. SECURITY DEFINER; EXECUTE service_role only.';

-- Reaffirm grants after REPLACE (same posture as 037/047)
